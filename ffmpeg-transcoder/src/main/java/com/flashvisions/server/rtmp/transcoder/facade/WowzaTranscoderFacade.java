...
import com.flashvisions.server.rtmp.transcoder.wowza.command.AbortTranscodeCommand;
import com.flashvisions.server.rtmp.transcoder.wowza.command.DoTranscodeCommand;

public class WowzaTranscoderFacade implements ITranscoderFacade {

	
private static Logger logger = LoggerFactory.getLogger(WowzaTranscoderFacade.class);
	...
	public static ITranscoderFacade getInstance()
	{
		if(instance == null)
		{
			synchronized (WowzaTranscoderFacade.class){
				if(instance == null){
					instance = new WowzaTranscoderFacade();
				}
			}
		}
